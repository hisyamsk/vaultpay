...
- [x] Classify a definitive receiver-credit failure separately from transient database errors.
- [x] On a definitive receiver-credit failure, call the existing atomic failed-payment operation with a stable safe error code so the sender refund, refund ledger entry, `failed` status, and `payment.failed` event commit together.
- [x] Return success only after the failure/refund transaction commits so the RabbitMQ adapter acknowledges afterward.
...
- [x] A definitive receiver-credit failure invokes the existing failed-payment operation once with the expected error code.
- [x] A failure/refund transaction error is returned for retry and is not treated as successful finalization.
